...
var finance = localhost + '/purchase/order/finance/list'

var pagination = new Pagination(finance, "page", function (data) {
    fillTableData(data)
});
...
function fillTableData(data){
    console.log(data);
            ...
            for (var i = 0; i < data.length; i++) {
                    ...
                    for(var g=0;g<purchaseOrderGoods.length; g++){
                         doc+='<span>'+'<i style="display:inline-block;width:47px;margin-left:-17px">'+purchaseOrderGoods[g].totalCount+'</i>'+'*'+purchaseOrderGoods[g].totalPrice+'</span>'
                    ...
                    doc+='</tr>'
                    //doc +='<tr>'+'<td colspan="4" style="text-align:left;line-height:35px">'+'总计'+':'+data[i]. purchaseCount + ' </td>' +'<td colspan="5" style="text-align:left;line-height:35px">'+'总计'+':'+data[i].totalPrice + ' </td>' +'</tr>'
                    doc +='<tr>'+'<td colspan="4" style="text-align:left;line-height:35px">'+'总计'+'</td>'+'<td  colspan="2" >'+data[i].purchaseCount+'&nbsp;&nbsp;&nbsp;&nbsp;'+'¥'+data[i].totalPrice +'</td>'+'<td colspan="6"></td>'+'</tr>'
 ...
 //点击否关闭遮罩层
    ...
    $("#table_data").delegate(".approve_refuse",'click',function(){
            var url= "http://172.16.39.70:8005/purchase/order/finance/auditing";
            var orderNum =  $('.approve_agree').attr('data-orderNum') //获取当前订单编号
