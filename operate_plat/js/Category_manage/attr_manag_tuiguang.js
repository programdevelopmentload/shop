    ...
    if (data.length == 0) {   //没有资源时
        ...
        for (var i = 0; i < data.length; i++) {
            content.append(
                ...
                '<td>' + necessaryDraw(data[i].necessary) + '</td>' +       //是否必填
                '<td>' + stateDraw(data[i].state) + '</td>' +       //开关状态汉字渲染
                // '<td data-id="' + data[i].id + '" data-state="' + data[i].state + '" data-name="' + data[i].name + '"data-type="' + data[i].type + '">' + ' <a class="_look sm btn-sm btn-primary" href="./attr_manag_detail.html?id=' + data[i].id + '&name=' + data[i].name + '">查看</a><button class="_change btn-sm btn-warning" onclick="grade_Rename(this)">编辑关联分类</button><button class="_open btn-sm btn-success" onclick="grade_Switch(this,attr_switchUrl)">' + ctr_Switch(data[i].state) + '</button><button class="_close sm btn-danger btn-sm" onclick="grade_Delete(this,attr_delUrl)">删除</button>' + '</td>' +       //操作
                '<td data-id="' + data[i].id + '" data-state="' + data[i].state + '" data-name="' + data[i].name + '"data-type="' + data[i].type + '">' + ' <a class="_look page_miniBtn" href="./attr_manag_detail.html?id=' + data[i].id + '&name=' + data[i].name + '">查看</a><a href="./attr_manag_edit.html?type=' + data[i].type + '&id=' + data[i].id + '" class="_change page_miniBtn">编辑关联分类</a><button class="_open page_miniBtn hide_ctr_absolute" onclick="grade_Switch(this,attr_switchUrl)">' + ctr_Switch(data[i].state) + '</button><span class="_close page_miniBtn hide_ctr_absolute" onclick="grade_Delete(this,attr_delUrl)">删除</span>' + '</td>' +       //操作
                '</tr>'
            );
        }
        ;
