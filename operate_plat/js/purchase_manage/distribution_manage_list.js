...
//spu分页地址
var xinpinfenxiao_pageUrl = localUrl + '/goods/sku/distributionList';
//右侧区域表格店铺列表拉取
var shop_listUrl = zmy_localUrl + '/shop/findStartStateShopByShopCity';
...
function fillTableData(data) {
    ...
    for (var i = 0; i < resData.length; i++) {
        ...
        doc += "<td class='goods_model'><span>" + resData[i].brandName + "</span><span>" + resData[i].leafClassName + "</span><span>" + resData[i].model + "</span></td>>";

        doc += "<td>" + resData[i].service.goodsType + "</td>>";
        doc += "<td class='right_box' colspan='4'>";
        ...
        for (var j = 0; j < skuInfo.length; j++) {
            ...
            doc += "</div>";
            doc += "</div>";
            doc += "<div class='fl right_con'>" + skuInfo[j].sortNumber + "</div><div class='fl right_con'>" + draw_updateTime(skuInfo[j].createTime) + "</div><div class='fl right_con'>" + skuInfo[j].price + "</div></div>";
        }
        doc += "</td>";
        doc += "</tr>";
    }
...
function getData() {
    ...
    $.ajax({
        type: 'post',
        url: xinpinfenxiao_pageUrl,
        // url: '../../html/purchase_manage/fenxiao_liebiao.json',
        data: _pull_data,
        beforeSend: function () {
