    ...
    if (!data) {   //没有资源时
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            content.append(
                ...
                '<td>' + data[i].applyType + '</td>' +        //公司名称

                '<td>' + data[i].applyTime + '</td>' +       //所在地区
                '<td>资质审核</td>' +       //审核类型

                '<td>' + data[i].evaPerson + '</td>' +       //申请时间
                '<td>' + data[i].evaPerson + '</td>' +       //审核时间
                '<td>' + data[i].evaPerson + '</td>' +       //审核人

                '<td>' + data[i].evaResult + '</td>' +       //审核状态
                '<td data-id="' + data[i].id + '" data-state="' + data[i].state + '" data-name="' + data[i].name + '">' + ' <a class="_look sm btn-sm btn-primary" href="./qualifications_verify_detail.html?id=' + data[i].companyId + '&log=1">查看</a></td>' +       //操作
                '</tr>'
            );
        }
        ;
