...
function fillTableData(data) {
    ...
    if (!data) {   //没有资源时
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            // str += '<td>' + data[i].sellerImage.applyTime + '</td>';      //申请时间
            str += '<td>' + data[i].imageEvaTime + '</td>';      //审核时间
            str += '<td>' + data[i].imageEvaPerson + '</td>';      //审核人
            str += '<td>' + stateDraw(data[i].imageState) + '</td>';     //审核状态

            str += '<td data-id="' + data[i].id + '">' + ' <a class="_look page_miniBtn" href="./authentication_verify_logo_detail.html?id=' + data[i].id + '&log=1">查看</a></td>';      //操作
            str += '</tr>';
        }
        ;
        content.html(str);
