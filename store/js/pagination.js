...
function Pagination(_url, _pageId, _callBack){
    ...
    function searchList(pageNo, params) {
        ...
        params["pageNum"] = pageNo;//当前页码
        // params["pageSize"] = pageSize;//每页多少行，目前写死
        // var url = 'http://172.16.40.153:8009/shop/findDisplayRecord';
        // var url = 'http://172.16.40.134:8003/seller/examineAndVerifyList';
        // debugger;
        $.ajax({
            type : "post",
            catch:false,
            ...
            success:function(data) {
                ...
                if(data.success == true){
                    // debugger;
                    returnParams["currentPageNo"] = data.data.pageNum;//当前页码
                    returnParams["pageSize"] = data.data.pageSize;//
                    returnParams["totalCount"] = data.data.totalCount;//
    ...
    function fillData(data) {
        ...
        fillPageData(data.totalCount,data.startIndex, data.pageNum, data.totalPage, data.isMore);
    }
    ...
    function fillPageData(totalCount,hasPreviousPage, currentPageNo, totalPageCount, hasNextPage){
        ...
        //有上一页
        if(hasPreviousPage){
            result += "<a href=\"javascript:void(0);\" onclick=\"javascript:" +
                ".searchList(" + (currentPageNo - 1) + ");\" class=\"prev\"></a>";
        }
        //前面的“...”
